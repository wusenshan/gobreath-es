...
`
	...
	// 导入 es
	if !strings.Contains(collapsed, `import es "github.com/wusenshan/gobreath-es"`) {
		t.Error("输出应导入 es")
	}
	// 生成 UserColumnSet
	...
	// 字段包含导出字段，不含 Internal（json:"-"）
	if !strings.Contains(collapsed, "Id es.ColExpr") {
		t.Error("应包含 Id 字段")
	}
	if !strings.Contains(collapsed, "Name es.ColExpr") {
		t.Error("应包含 Name 字段")
	}
	if !strings.Contains(collapsed, "Age es.ColExpr") {
		t.Error("应包含 Age 字段")
	}
	if strings.Contains(collapsed, "Internal es.ColExpr") {
		t.Error("不应包含 json:\"-\" 字段 Internal")
	}
	// 初始化语句
	if !strings.Contains(collapsed, `es.ColOf[User]("Id")`) {
